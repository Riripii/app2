...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya apikey? https//docs-api-Fhans.herokuapp.com/register'
    },
    notkey: {
        status: false,
        creator: `Fhans`,
    ...
    invalidKey: {
        ...
        creator: `Fhans`,
        code: 406,
        message: 'apikey beta silakan gunakan apikey anda klick link docs-api-Fhans.herokuapp.com/register'
    },
    invalidlink: {
        status: false,
        creator: `Fhans`,
    ...
    error: {
        status: false,
        creator: `Fhans`,
        message: 'emror creatornya tolol'
    }
}
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------FhansGanteng'+'Fhans--GANS';
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'FhansGanss') return res.json(loghandler.invalidKey)
...
router.get('/cekapikey', async (req, res, next) => {
	...
	json = JSON.stringify({
		...
		message: `jangan lupa follow Fhans`
	})
} else {
	json = JSON.stringify({
		status: false
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
...
router.get('/infonpm', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
    if (!query) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter query"})
...
router.get('/nulis', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
    if (!text) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter text"})

       fetch(encodeURI(`http://zekais-api.herokuapp.com/bukukiri?text=${text}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/hadits', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
    if (!kitab) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter kitab"})
    if (!nomor) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter nomor"})
...
router.get('/quran', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
    if (!surah) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter surah"})
    if (!ayat) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter ayat"})
...
router.get('/fbdown', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
    if (!url) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter url"})
...
router.get('/wikipedia', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
        if(!search) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter search"})
...
router.get('/jadwalshalat', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
        if(!kota) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter kota"})
...
router.get('/tts', async (req, res, next) => {
	...
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
       ...
       fetch(encodeURI(`http://zekais-api.herokuapp.com/speech?lang=id&text=${text}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/hilih', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
        if(!kata) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter kata"})
...
router.get('/liriklagu', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
        if(!lagu) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter kata"})
...
router.get('/infocuaca', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
	if(!provinsi) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter provinsi"})
...
router.get('/translate', async (req, res, next) => {
	...
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
	if(!kata) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter kata"})
       fetch(encodeURI(`http://zekais-api.herokuapp.com/translate?text=${kata}&lang=id`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/random/wallpaper', async (req, res, next) => {
	...
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
       ...
       fetch(encodeURI(`http://zekais-api.herokuapp.com/randomwp`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/news/cnn', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
    if (!type) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter type"})
...
router.get('/filmapik/search', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
    if (!film) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter film"})

       fetch(encodeURI(`http://zekais-api.herokuapp.com/filmapik?query=${film}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/filmapik/play', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
    if (!id) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter id"})
...
router.get('/lk21/year', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
    if (!tahun) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter tahun"})
...
router.get('/lk21/country', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
    if (!negara) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter negara"})
...
router.get('/lk21/genre', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
    if (!tipe) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter tipe"})
...
router.get('/ig/stalk', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
    if (!username) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter username"})
...
router.get('/yutub/search', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FhansGanss') return res.json(loghandler.invalidKey)
    if (!video) return res.json({ status : false, creator : `Fhans`, message : "masukan parameter video"})
